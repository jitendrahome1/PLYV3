...
//https://github.com/bryx-inc/BRYXBanner/tree/7ae4e637730bf1fd3d09ec6bf80af0f39693ede1

import UIKit

private enum BannerState {
    case showing, isHidden, gone
}
...
open class Banner: UIView {
    ...
    fileprivate var bannerState = BannerState.isHidden {
        didSet {
    ...
    fileprivate func forceUpdates() {
        guard let superview = superview, let showingConstraint = showingConstraint, let isHiddenConstraint = isHiddenConstraint else { return }
        switch bannerState {
        case .isHidden:
            superview.removeConstraint(showingConstraint)
            superview.addConstraint(isHiddenConstraint)
        case .showing:
            superview.removeConstraint(isHiddenConstraint)
    ...
    fileprivate var isHiddenConstraint: NSLayoutConstraint?
    ...
    override open func didMoveToSuperview() {
        ...
        switch self.position {
            case .top:
                ...
                isHiddenConstraint = self.constraintWithAttribute(.bottom, .equal, to: .top, of: superview, constant: yOffset)
            case .bottom:
                showingConstraint = self.constraintWithAttribute(.bottom, .equal, to: .bottom, of: superview)
                let yOffset: CGFloat = 7.0 // Offset the bottom constraint to make room for the shadow to animate off screen.
                isHiddenConstraint = self.constraintWithAttribute(.top, .equal, to: .bottom, of: superview, constant: yOffset)
        }
    }
