...
- (void)done {
    ...
    if ([delegate respondsToSelector:@selector(hudWasisHidden:)]) {
        [delegate performSelector:@selector(hudWasisHidden:) withObject:self];
    }
}
...
- (void)setupViews {
    ...
    [topSpacer setHidden:YES];
    [bezelView addSubview:topSpacer];
    _topSpacer = topSpacer;

    UIView *bottomSpacer = [UIView new];
    bottomSpacer.translatesAutoresizingMaskIntoConstraints = NO;
    [bottomSpacer setHidden:YES];
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        BOOL firstVisible = !firstView.isHidden && !CGSizeEqualToSize(firstView.intrinsicContentSize, CGSizeZero);
        BOOL secondVisible = !secondView.isHidden && !CGSizeEqualToSize(secondView.intrinsicContentSize, CGSizeZero);
